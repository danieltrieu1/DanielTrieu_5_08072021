...
let products = [];
let productInLocalStorage = JSON.parse(localStorage.getItem("product")); // Récupère les données en convertissant la chaine de caractère JSON en objet javascript
...
if (productInLocalStorage === null || productInLocalStorage == 0) {
  ...
  for (i = 0; i < productInLocalStorage.length; i++) {
    products.push(productInLocalStorage[i].id); // La méthode .push() récupère uniquement l'id de chaque produit dans le panier, dans le array products
    
    // console.log(products.length);
    // Ce code va s'incrémente à chaque tour de boucle / Pour chaque produit qui a été ajouté au panier
    productsCart =
      productsCart + // Concaténation pour que le produit s'ajoute au tableau productsCart sans écraser le précédent 
      `  
  ...
  deleteProduct = () => {
    ...
    for (let a = 0; a < deleteItem.length; a++) {
      deleteItem[a].addEventListener("click", (event) => {
        ...
        localStorage.setItem("product", JSON.stringify(productInLocalStorage)); // Traduit l'objet javascript en chaine de caractère JSON

        // Création d'une alerte pour dire que le produit a bien été supprimé et actualisation de la page
        alert("Votre article a bien été retiré de votre panier !");
        window.location.href = "cart.html"; // Rechargement de la page pour actualiser le contenu du panier
      });
    }
  };
  ...
  priceAmount = () => {
    ...
    for (m = 0; m < productInLocalStorage.length; m++) {
      ...
      const reducer = (Accumulator, currentValue) => Accumulator + currentValue; // Accumulator = La valeur précédente
  ...
  totalArticles = () => {
    ...
    for (e in productInLocalStorage) {
      // Analyse et converti la valeur 'quantité' dans le localstorage en une chaîne, et renvoie un entier (parseInteger), sur la base décimale de 10
...
form = () => {
  ...
  order.addEventListener("click", (event) => {
    ...
    formFirstName = () => {
      ...
      if (/^[a-zA-Z--]{2,20}$/.test(validFirstName)) { //Regex contenant l'ensemble des caractères admis dans le champs de remplissage à la validation
        return true;
      } else {
    ...
    formCheck = () => {
      if (
        ...
        // localStorage.setItem('contact', JSON.stringify(contact)); // donnée utilisateur
        return true;
      } else {
